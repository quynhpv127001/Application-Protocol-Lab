...
- **Tài liệu API copy-paste được luôn**: Mọi API, Endpoint hoặc Kết nối được khai báo BẮT BUỘC phải đi kèm document (nằm trong `README.md` hoặc `api.md`). Format docs phải chi tiết, chuẩn mực (có bảng Headers, Params, Request Body, Response mẫu cho case thành công/thất bại).
- **Văn phong Document siêu ngắn gọn**: Tuyệt đối không viết những câu giải thích rườm rà, thừa thãi, phân bua lý do (ví dụ: "Tại sao không nên dùng...", "Nếu bắt buộc phải..."). Document phải cực kỳ ngắn gọn, sạch sẽ, đi thẳng vào các thông số cấu hình và các bước 1, 2, 3... để copy-paste là chạy ngay lập tức.
...
- [x] **2.3 The Backend Engineer way**:
    ...
    - [x] **Handshake Auth**: Cấu hình ChannelInterceptor xác thực bằng `login` (Username) và `passcode` (Password) lúc thiết lập `ws://`. Đảm bảo tương thích chuẩn với mọi Extension STOMP Client.
...
- [x] **3.1 Setup Module**: Khởi tạo `p3_socketio`, tích hợp thư viện `netty-socketio`.
- [x] **3.2 The Frontend-Backend Connection**:
    - [x] Tính năng Auto-fallback (Long-Polling <-> WebSocket).
    - [x] Quản lý Room/Namespaces tự động.
- [x] **3.3 Hoàn thiện Module**: Test & `README.md` (kèm code socket.io-client mẫu để copy).
- [x] **3.4 Giao diện UI**: Tích hợp tab Socket.IO, kiểm thử Auto-fallback.
